...
export const confirmPaymentIntent = createAsyncThunk(
  ...
  async (values: any, thunkAPI) => {
    try {
      ...
      const {
        data: { result },
      } = await http.post<{ result: TransactionResponse }>(
        url,
        transactionData
      );
      return result;
...
export const saveContactUs = createAsyncThunk(
  "user/saveContactUs",
  async (values: ContactUsData, thunkAPI) => {
    try {
      const url = Apiconfig.endpoints.auth.saveContactUs;

      let contactus = JSON.parse(JSON.stringify(values));

      const {
        data: { result },
      } = await http.post<{ result: boolean }>(url, contactus);
      return result;
    } catch (err) {
      return thunkAPI.rejectWithValue(err);
    }
